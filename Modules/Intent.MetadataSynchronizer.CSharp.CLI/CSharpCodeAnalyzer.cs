...
{
    public static async Task<CoreTypesData> ImportMetadataFromFolder(CSharpConfig csConfig)
    {
        var hintHelper = new HintHelper();
        var syntaxTrees = await ReadSyntaxTreesFromFolder(csConfig, hintHelper);
        var compilation = CreateCompilationForFullTypeResolution(syntaxTrees);
        return AnalyzeCSharpCoreTypesInSyntaxTrees(hintHelper, compilation, syntaxTrees);
    }

    private static async Task<IReadOnlyList<SyntaxTree>> ReadSyntaxTreesFromFolder(CSharpConfig csConfig, HintHelper hintHelper)
    {
        ...
        await ReadDomainEntities(syntaxTrees, visited, csConfig, hintHelper, options);
        ...
        static async Task ReadDomainEntities(List<SyntaxTree> syntaxTrees, HashSet<string> visited, CSharpConfig csConfig, HintHelper hintHelper,
            EnumerationOptions options)
        {
            IEnumerable<string> csharpFiles = [];
            if (!string.IsNullOrWhiteSpace(csConfig.DomainEntitiesFolder))
            {
                csharpFiles = Directory.EnumerateFiles(csConfig.DomainEntitiesFolder, "*.cs", options);
            }
            ...
            foreach (var file in csharpFiles)
            {
                ...
                var syntaxTreeWithHint = hintHelper.ApplyHintToSyntaxTreeRoot(syntaxTree, HintHelper.HintDomainEntities);
        ...
        IReadOnlyList<SyntaxTree> syntaxTrees)
    {
        ...
        ExtractClassesFromSyntaxTrees(hintHelper, compilation, syntaxTrees, data);
        ExtractRecordsFromSyntaxTrees(hintHelper, compilation, syntaxTrees, data);
        ExtractEnumsFromSyntaxTrees(hintHelper, compilation, syntaxTrees, data);
        ...
        CoreTypesData data)
    {
        ...
        ExtractPartialClassesFromSyntaxTrees(hintHelper, compilation, classDeclarations, data);
        ExtractNonPartialClassesFromSyntaxTrees(hintHelper, compilation, classDeclarations, data);
            ...
            CoreTypesData data)
        {
            ...
            foreach (var group in groupedClassDeclarations)
            {
                ...
                ApplyDataForClassesAndRecords(hintHelper, firstClassDeclaration, data, classData);
            }
        }

        static void ExtractNonPartialClassesFromSyntaxTrees(HintHelper hintHelper,
            CSharpCompilation compilation,
            List<ClassDeclarationSyntax> classDeclarations,
            CoreTypesData data)
        {
            ...
            foreach (var declarationSyntax in normalClassDeclarations)
            {
                ...
                ApplyDataForClassesAndRecords(hintHelper, declarationSyntax, data, classData);
            }
        }
    ...
    private static void ExtractRecordsFromSyntaxTrees(HintHelper hintHelper, CSharpCompilation compilation, IReadOnlyList<SyntaxTree> syntaxTrees, CoreTypesData data)
    {
        ...
        foreach (var declarationSyntax in recordDeclarations)
        {
            ...
            ApplyDataForClassesAndRecords(hintHelper, declarationSyntax, data, recordData);
        }
    }
    
    private static void ApplyDataForClassesAndRecords(HintHelper hintHelper, BaseTypeDeclarationSyntax typeDeclaration, CoreTypesData data, ClassData classData)
    {
